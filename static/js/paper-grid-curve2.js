...
window.onload = function () {
  ...
  function drawHintCurve(path, currentPoint) {
    ...
    var pointAt1stOffset =currentState.initialRawPoint;// path.getPointAt(firstOffset);
    var pointAt2ndOffset = path.getPointAt(secondOffset);
    var pointAt3rdOffset = currentPoint;//path.getPointAt(thirdOffset);

    var arc = new Path.Arc(pointAt1stOffset,pointAt2ndOffset,pointAt3rdOffset );
    arc.strokeColor = 'red';
...
// at the position of the mouse:
  tool.onMouseDrag = function (event) {
    if(currentState && currentState.path) {
      ...
      drawHintCurve(currentState.path, event.point);
